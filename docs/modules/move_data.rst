...
Ideally you take sharding in consideration at the moment you first conceptualise the project. And you might start to use this library from day one. In that case, your data will always be created on the shards you want them to, and add shards as you go.
...
Next is to move data away from that 'main' schema, and into their own schema's. After all, you won't see any benefit from sharding if you use only a single shard.
...
2. Put both shards into Maintenance mode.
...
We advice you to move data over to a new, empty shard. Or know for sure that there won't be id collisions.
If you want to command to create the target shard for you, you will discover it cannot do that out of the box. The library does not know what naming scheme you use, and what fields your Shard model might have in addition to the base. And if you use mapping models, there are even more considerations.
...
.. code-block:: python
    ...
    class Command(BaseCommand):
        def get_target_shard(self, root_object, options):
            ...
            """
            return Shard.objects.create(alis='new Shard', node=get_new_shard_node(), organization=root_object)
...
``--source_shard_alias``
~~~~~~~~~~~~~~~~~~~~~~~~
The ``--source_shard_alias`` argument should be the name of the shard the data is currently located.

``--target_shard_alias``
~~~~~~~~~~~~~~~~~~~~~~~~
The ``--target_shard_alias`` argument should be the name of the shard that will receive the data.
...
``--root_object_id``
~~~~~~~~~~~~~~~~~~~~
The ``--root_object_id`` argument should must be the id of the root_object.
...
``--no_input``
~~~~~~~~~~~~~~
The ``--no_input`` argument allows you to supress the confirmation the command will prompt.
